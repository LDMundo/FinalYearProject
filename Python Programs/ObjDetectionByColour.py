...
def extractMask(img):
    ...
    greenLower = np.array([34,  100,  75], np.uint8)     #lower range of green colour
    greenUpper = np.array([70, 255, 255], np.uint8)      #upper range of wood colour
    ...
    maskOpenMorph = cv2.morphologyEx(greenMask, cv2.MORPH_OPEN, kernelOpen)  #morphology opening
        ...
        if (message.decode('utf-8') == "req"):  # check if the message decoded is "request"
            ...
            for i in range(len(contours)):              #iterates through all the contours found
                ...
                if(area > 250):                         #if the area of the contour is greater than 250 px^2
                    ...
                    #check if the bottom y coordinate is greater than the current closest rectangle
                    #store the coordinates if it closer than the previous one
            ...
            if closestYplusH > 0:
                if closestYplusH >= yLimit:
                    ...
                    # check if objects are within xLimitLeft and Center
                    if (xOfInterest>xLimitLeft or xplusw>xLimitLeft) and (xOfInterest<xCenter or xplusw<xCenter):
                        print("turnRight\n")
                        ser.write('turnRight'.encode('utf-8'))  #send turn right command to arduino
                        ser.flush() 
                    # check if objects are within xLimitRight and Center      
                    elif (xOfInterest<xLimitRight or xplusw<xLimitRight) and (xOfInterest>xCenter or xplusw>xCenter):
                        print("turnLeft\n")
                        ser.write('turnLeft'.encode('utf-8')) #send turn left command to arduino
                        ser.flush() 
                #check if the object is too close                                                      
                elif (closestYplusH <= yLimit) and (closestYplusH >= yLimit-int(0.5*yLimit)):
                        print("reverse\n")
                        ser.write('reverse'.encode('utf-8')) #send reverse command to arduino
                        ser.flush() 
            else: #if no object are detected
                print("noObject\n")
                ser.write('noObject'.encode('utf-8'))
                ser.flush() 
        else: #if request is not "req"
            print("invalid\n")
